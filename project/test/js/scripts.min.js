...
const tabs = () => {
  document.querySelectorAll('.vars__tabs').forEach(item=>{
    item.addEventListener('click',e=>{
      if((e.target.classList.contains('vars__nav-btn')) && !(e.target.classList.contains('active'))){
        item.querySelectorAll('.vars__nav-btn').forEach(itesa=>{itesa.classList.remove('active')});
        e.target.classList.add('active')
        item.querySelectorAll('.vars__block-item').forEach(itesa=>{itesa.classList.remove('active')});
...
const header = () => {
  const addHeaderAdapt = () => {
    if(window.screen.width <= 1200 && window.screen.width > 720){
