...
function runRow(){
  console.log(dur);
  var i=0;
  mq=$('.marquee').marquee({speed:dur,delayBeforeStart:0,}).bind('finished',function(e){if($('#last_run_element').offset().left<=(-130)){mq.marquee('destroy');$('.marquee span').hide();$('.marquee .jackpot-view').show(200);setTimeout(function(){$('.marquee span').each(function(){if($(this).hasClass('jackpot-view')){$(this).hide()}else{$(this).show();}});runRow();},60000);}});
}
...
function tabsBigs(){
  $('#topay').on('click',function(){
    $('#logAcc').removeClass('active');
    $('#topay-block').addClass('active');
    playSound('click_btn');
  });
  $('#navConditions').on('click',function(){
    $('#logAcc').removeClass('active');
    $('#logPass').addClass('active');
    playSound('click_btn');
  });
  $('#reg-page-btn').on('click',function(){
    $('#logAcc').removeClass('active');
    $('#logNew').addClass('active');
    playSound('click_btn');
  });
  $('.back-chat-log').on('click',function(){
    $('.popup-wind__ollAcc').removeClass('active');
    $('#logAcc').addClass('active');
    playSound('click_btn');
  });
  $('.back-chat-set').on('click',function(){
    $('.popup-wind__wsa').removeClass('active');
    $('#setMain').addClass('active');
    playSound('click_btn');
  });
  $('#navRol').on('click',function(){
    $('#setMain').removeClass('active');
    $('#setBeck').addClass('active');
    playSound('click_btn');
  });
  $('#rollback-link-action').on('click',function(){
    $('#setBeck').removeClass('active');
  ...
  $('#navAbout').on('click',function(){
    $('#setMain').removeClass('active');
    $('#setAbout').addClass('active');
    playSound('click_btn');
  });
  $('#to_rollback_page_link').on('click',function(){
    $('#setAbout').removeClass('active');
    $('#setBeck').addClass('active');
    playSound('click_btn');
  });
  $('#to_double_cash_page_link').on('click',function(){
    $('#setAbout').removeClass('active');
    $('#setAct').addClass('active');
    playSound('click_btn');
  });
  $('#to_second_chance_page_link').on('click',function(){
    $('#setAbout').removeClass('active');
    $('#setAct').addClass('active');
    playSound('click_btn');
  });
  $('#to_action_page_link').on('click',function(){
    $('#setAbout').removeClass('active');
    $('#setAct').addClass('active');
    playSound('click_btn');
  });
  $('#navHist').on('click',function(){
    $('#setMain').removeClass('active');
    $('#setHist').addClass('active');
    playSound('click_btn');
  });
  $('#navSet').on('click',function(){
    $('#setMain').removeClass('active');
    $('#setSet').addClass('active');
    playSound('click_btn');
  });
  $('#navConditions').on('click',function(){
    $('#setMain').removeClass('active');
    $('#setConditions').addClass('active');
    playSound('click_btn');
  });
  $('#navPolitic').on('click',function(){
    $('#setMain').removeClass('active');
    $('#setPolitic').addClass('active');
    playSound('click_btn');
  });
}
...
function ckecbox(){
  // $('.topay-block__labest').on('click', ()=>{
  //   $(this).find('input').prop( "checked", true )
  //   console.log($(this).find('.okas').is(':checked'));
  //   console.log($(this).find('.okas'));
  // })
  let items = document.querySelectorAll('.topay-block__labest')
  items.forEach(item=>{
    item.addEventListener('click', ()=>{
      if(item.querySelector('input').checked == true){
        item.querySelector('input').checked = false;
        item.querySelector('.popup-wind__input').classList.remove('active');
      } else {
...
const init = () => {
  ...
  sliderInit();
  document.querySelector('.top-box__position').style.height = document.querySelector('.top-box__wrapper').offsetHeight - document.querySelector('.nav-bar').offsetHeight - document.querySelector('.top-box__slider-wrap').offsetHeight - 38 + 'px';
  document.querySelector('.top-box__slider-wrap').style.height = heightCard + 'px';
  // console.log(document.querySelector('.top-box__slider-wrap').offsetHeight);
  ollAddNavBtn();
  codesCheck();
  tabsBigs();
  playMusic();
